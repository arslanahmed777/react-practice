...
const Validator = (data, type, Error) => {
    ...
    for (let i = 0; i < type_arr.length; i++) {
        if (typeof type_arr[i] === "function") {
            error = type_arr[i](data_arr) ? (<div style={styling}>{error_arr[i]}</div>) : "";
        }
        else {
            if (type_arr.includes(ValidationTypes.required) && data_arr[0] === "") {
                error = (<div style={styling}>{error_arr[type_arr.indexOf(ValidationTypes.required)]}</div>);
                break;
            }
            if (type_arr.includes(ValidationTypes.capitalize) && data_arr[0] !== data_arr[0].toUpperCase()) {
                error = (<div style={styling}>{error_arr[type_arr.indexOf(ValidationTypes.capitalize)]}</div>);
                break;
            }
            if (type_arr.includes(ValidationTypes.startEndDate)) {
                if (data[0] === "" || data[1] === "") break
                if (Date.parse(data[0]) > Date.parse(data[1])) {
                    error = (<div style={styling}>{error_arr[type_arr.indexOf(ValidationTypes.startEndDate)]}</div>);
                    break;
                }
            }
        }
